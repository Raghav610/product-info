...
public class RequestValidator {
	...
	public boolean isValidRequest(ProductItem productItem) throws MissingParameterError, BadRequestException {
		...
		if(isValidReq) {
			if(!StringUtils.isNumeric(productItem.getSKU()) || CommonUtil.isValidLength(productItem.getSKU(), ProductConstants.MAX_LENGTH, ProductConstants.MIN_LENGTH)) {
				throw new BadRequestException(new ErrorInfo(HttpStatus.BAD_REQUEST,messageSource.getMessage(ProductConstants.NUMERIC_FIELD_REQUIRED, new Object[] { ProductConstants.FIELD_SKU }, Locale.US)));
			}
			if(!CommonUtil.isNumeric(productItem.getQuantity())) {
	...
	private static boolean isNotNullCheck(ProductItem productItem) throws MissingParameterError {
		...
		}else if (CommonUtil.isEmpty(productItem.getWarehouseCode())) {
			throw new MissingParameterError(ProductInfoException.GROUP.GENERIC, "Ware house Code is null or empty");
		}
		return true;
	}
}
